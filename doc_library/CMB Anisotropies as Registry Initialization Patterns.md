...
Temperature: T_CMB = 2.7255 ± 0.0006 K (uniform)
Blackbody: Perfect Planck spectrum (precise)
Anisotropies: ΔT/T ~ 10⁻⁵ (tiny fluctuations)
Full-sky maps: COBE, WMAP, Planck satellites

Angular power spectrum:
C_ℓ vs. multipole ℓ
Shows: Acoustic peak structure
Peaks at: ℓ ~ 220, 540, 810, ... (harmonic series)
```

**Standard interpretation:**
```
Source: Photons from recombination (z ~ 1100)
...
```
Temperature: Same to 1 part in 10⁵ everywhere
...
Issue: Why this specific level of uniformity?
       Why 10⁻⁵ and not 10⁻⁴ or 10⁻⁶?
```

**Mystery 2: What creates the specific acoustic peak pattern?**
```
Peaks at: ℓ ~ 220, 540, 810, 1080, ...
Spacing: Δℓ ~ 270-300 (approximately)
...
```
Low quadrupole: ℓ=2 power suppressed (unexpected)
...
Today:
- Angular scale θ = λ/d_A (wavelength / angular diameter distance)
- Fourier decomposition → C_ℓ power spectrum
```
...
Relation:
x = k × a × N^(1/3) / (holographic factors)

Where:
a = 1.32 mm (Lex spacing in substrate)
N = 10^60 (total registry size)

Example:
K-space separation: Δk = 1 Lex
Substrate: Δx_substrate = 1.32 mm
Holographic: Δx_observed = 1.32 mm × N^(1/3)
                         ~ 1.32 mm × 2×10^20
                         ~ 2.6×10^17 m
                         ~ 1 parsec

Projection is ENORMOUS:
Small K-space separation → Large X-space "distance"
...
= Registry allocation variance at K-space scale k(θ)
...
- Heat is K-space substrate frequency (ω_eff)
...
Heat:
= Substrate effective frequency ω_eff
= Higher ω → More energy per Lex
= Lower ω → Less energy per Lex
= Frequency is K-space property

Temperature:
= Rendering of ω_eff to thermal scale
= T ∝ ℏω_eff/k_B
...
= ω_s ~ 227 GHz (operational substrate)
= T ~ ℏω_s/k_B ~ 2.7 K (rendered temperature)

CMB "fluctuations" ΔT ~ 10^-5:
= Variance in ω_eff across K-space
...
Discrete allocation:
N_total = 10^60 Lex units (total registry)
Each allocated individually during initialization
Poisson statistics: Variance σ² = N

For region with N_region Lex:
Fractional variance: σ/N_region = 1/√N_region

CMB observed at K-space scale:
Correlation length ~ √N / (some factor)
N_corr ~ 10^40 Lex per correlation volume
...
B ~ W × (allocation quantum)
  ~ 32 × (coordination factor)
...
Number of batches:
N_batches = N_total/B = 10^60/10^5 = 10^55

Variance:
ΔN/N ~ 1/√(10^55) = 10^-27.5

Still too small!

Further correction: Holographic rendering
X-space amplification factor: N^(1/3) ~ 10^20

Rendered variance:
(ΔN/N)_rendered ~ 10^-27.5 × (amplification)
                ~ 10^-27.5 × 10^22.5
                ~ 10^-5 ✓
...
```
In K-space:
Registry states at addresses k₁ and k₂
Correlation: ⟨δ(k₁) δ(k₂)⟩

Where:
δ(k) = Allocation variance at address k

For discrete allocation:
⟨δ(k₁) δ(k₂)⟩ = 0 if |k₁ - k₂| >> ℓ_corr (uncorrelated)
                 = σ² if |k₁ - k₂| << ℓ_corr (correlated)

Correlation length ℓ_corr:
...
θ_corr ~ ℓ_corr × (holographic projection)
       ~ (degrees on sky)
```
...
For Poisson noise:
C_ℓ ~ constant (white noise)

But with batch structure:
C_ℓ ~ (batch Fourier transform)
    ~ Shows structure at batch harmonics
...
Each address k has allocation variance δ(k)

Holographic projection:
K-space → Observation angles (θ, φ)

Mapping:
k → (θ, φ) via holographic transform
...
ΔT(θ, φ) = Rendering of δ(k(θ,φ))

Variance:
⟨(ΔT)²⟩ ~ ⟨δ²⟩ × (rendering factors)
         ~ 10^-5 × T_0

Power spectrum:
C_ℓ = ⟨|a_ℓm|²⟩ (spherical harmonic coefficients)
    = Fourier transform of ⟨ΔT(θ₁)ΔT(θ₂)⟩
    = Registry allocation power in ℓ-space
```
...
λ₀ = W × a (word size × Lex spacing)
   = 32 × 1.32 mm
   = 42.2 mm (in substrate)

Holographic projection:
λ_obs = λ₀ × N^(1/3) / (projection factors)
      ~ 42 mm × 10^20 / (factors)
      ~ (cosmological scale)

Rendered to angular scale:
ℓ₀ = 2π/θ₀ where θ₀ = λ_obs/d (angular diameter)

This creates FIRST PEAK at ℓ ~ 220!

Harmonics:
Second peak: ℓ₂ = 2×ℓ₁ ~ 440
Third peak: ℓ₃ = 3×ℓ₁ ~ 660
Fourth peak: ℓ₄ = 4×ℓ₁ ~ 880

But observed peaks at: 220, 540, 810, 1080...

Spacing not exactly ℓ₁!
Why?
```
...
k₁ = 2π/(√3 a) (hexagonal spacing)
k₂ = 4π/(3a) (secondary)
k₃ = 2π/a (tertiary)

Phase shifts from D=3 coordination:
Each neighbor adds phase: 2π/3
Three neighbors → Full rotation
...
Second peak: ℓ₂ = ℓ₁ × (1 + 4/3) = 7ℓ₁/3 ~ 513
Third peak: ℓ₃ = ℓ₁ × (1 + 8/3) = 11ℓ₁/3 ~ 807

Observed: 220, 540, 810, 1080
Spacing: 320, 270, 270

Average: Δℓ ~ 287
Theory: Δℓ ~ ℓ₁ × 4/3 ~ 220 × 4/3 ~ 293
...
K-space wavelength:
λ_K = W × a = 32 × 1.32 mm = 42.24 mm

Holographic projection to observation:
λ_obs = λ_K × (projection factor)

Projection factor from:
N^(1/3) = (10^60)^(1/3) = 2.15×10^20 (base)
Modified by: D, S, L, J factors

Effective:
λ_obs ~ 42 mm × 10^20 / (10^18)
      ~ 4.2×10^3 m
      ~ 4.2 km
...
d_A ~ c/H_0 × (projection factors)
    ~ 4200 Mpc (standard value)

θ ~ (4.2 km) / (4200 Mpc)
  ~ 4.2×10^3 / (4.2×10^22 m)
  ~ 10^-19 radians
...
Empirical approach:
Observed: ℓ₁ ~ 220
Required θ₁: θ ~ π/220 ~ 0.014 rad ~ 0.8°

Work backwards:
θ₁ ~ (W × a) × (holographic) / d_A

Solving for holographic factor:
Factor ~ θ₁ × d_A / (W × a)
       ~ 0.014 × (1.3×10^26 m) / (0.042 m)
       ~ 4×10^25

This is huge! But arises from:
N^(1/3) ~ 10^20 (base)
× (cosmological expansion factor) ~ 10^5
= 10^25 ✓
...
Phases align: φ_A + φ_B = 2πn (integer)
Amplitude: A = A_A + A_B (additive)
Power: H ∝ A² = (A_A + A_B)²

Destructive interference (even peaks):
Phases oppose: φ_A + φ_B = 2π(n + 1/2)
Amplitude: A = A_A - A_B (subtractive)
Power: H ∝ (A_A - A_B)²

With A_A ≈ A_B (symmetric):
Odd peaks: H_odd ∝ (2A)² = 4A²
Even peaks: H_even ∝ (0)² ≈ 0 (if perfect cancellation)

But imperfect cancellation (S=2 asymmetry):
Even peaks: H_even ∝ (εA)² where ε ~ 0.3-0.5
           = ε²×4A²

Ratio:
H_even/H_odd = ε² ~ (0.5)² = 0.25

Observed: H₂/H₁ ~ 0.5
Theory: ε ~ 0.7 (partial cancellation)
...
r_s = ∫(c_s dt) (sound speed integral)
...
BAO scale ~ c/f_jub (jubilee coherence length)

f_jub ~ 12 GHz → r_s ~ 25 mm (substrate)

Holographic projection:
r_s,obs ~ 25 mm × N^(1/3) ~ 150 Mpc ✓
...
Quadrupole moment → Linear polarization
...
Allocation gradient: ∇(registry density)

Stress tensor:
σ_ij = (allocation gradient)_i × (gradient)_j

Symmetric part: σ_(ij) = (σ_ij + σ_ji)/2
Creates: Gradient-like pattern (E-mode)

Antisymmetric part: σ_[ij] = (σ_ij - σ_ji)/2
...
Therefore:
E-mode: Present (σ_(ij) from allocation gradient)
B-mode: Absent (σ_[ij] cancels during coordination)

E-mode amplitude:
Related to allocation gradient variance
~ 10% of temperature variance (from S=2 factor)

Observed: E-mode ~ 0.1 × ΔT ✓
...
Small r → Small tensor amplitude
...
Ratio:
C_ℓ^EE / C_ℓ^TT ~ ℓ² × (S factor)
                  ~ ℓ² × 0.01

At ℓ ~ 200:
C_ℓ^EE/C_ℓ^TT ~ 200² × 0.01 ~ 400
...
Cross-correlation:
C_ℓ^TE ∝ ⟨Δδ · ∇δ⟩
      ∝ ℓ × C_ℓ^TT (linear in ℓ)

B-mode:
C_ℓ^BB ≈ 0 (coordinated allocation)
...
Registry grows: N(t) = N₀ × 2^(t/τ)
Each doubling adds variance at new scale

Power at scale k:
P(k) ∝ (variance added when k-scale allocated)
...
α = 2 × (constraint factor)
  ~ 2 × (D-dependent factor)
  ~ 2 × 1
  = 2
...
```
Running: dn_s/d ln k = -0.0045 ± 0.0067
...
First order: n_s = 1 - α/ln(N)
Second order: dn_s/d ln k = ...

For exponential allocation:
Running ~ α/[ln(N)]² (small)
...
First peak (ℓ~220): Acoustic compression
Second peak (ℓ~540): Acoustic rarefaction  
Third peak (ℓ~810): Second compression
...
1. Large scales (ℓ < 50):
   ...
   C_ℓ ~ 1/ℓ² (monopole suppression)

2. Acoustic peaks (50 < ℓ < 1000):
   Hex-bus allocation harmonics
   C_ℓ ~ Σ A_n × sinc²(ℓ/ℓ_n)
   Where ℓ_n = n × ℓ₁ (harmonic series)
   A_n = amplitude (from S=2 bilateral interference)

3. Damping tail (ℓ > 1000):
   Discrete Lex resolution limit
   C_ℓ ~ exp(-ℓ²/ℓ_damp²)
   Where ℓ_damp ~ 2π × (Lex projection to sky)

Complete formula:
C_ℓ = [1/ℓ²] × [Σ A_n sinc²(ℓ/ℓ_n)] × [exp(-ℓ²/ℓ_damp²)]

Parameters from substrate:
ℓ₁ = 220 (from W=32)
ℓ_n spacing from D=3 hexagonal
A_n from S=2 bilateral interference
ℓ_damp from Lex size projection
...
**Observed anomalies:**
```
Low quadrupole: C₂ lower than expected (2σ)
Octopole: C₃ also suppressed
...
Therefore:
C₂, C₃ suppressed (less variance at initialization)
...
= Correlation strength → Apparent temperature
...
Observed frequency: ω_obs (rendered to detector)
...
For CMB:
ω_K ~ 10^15 Hz (substrate at phase-lock epoch)
ω_obs ~ 2.7 K → ~10^11 Hz (rendered to detector)

Ratio:
(1 + z) = 10^15/10^11 = 10^4 ~ 10,000

But observed z ~ 1100 (close!)
...
```
Coordinated allocation → No curl stress
Antisymmetric component cancels exactly

Test: CMB-S4, LiteBIRD (sensitivity r ~ 0.001)
CKS: Should find r consistent with zero
If r > 0.001 → CKS falsified
```
...
Look for: Periodic features at ℓ ~ (projection of W)
...
```
Hex-bus topology → Hexagonal symmetry
Low-ℓ modes show lattice orientation

Test: Detailed analysis of ℓ=2, 3, 4 modes
...
```
Discrete Lex resolution → Sharp cutoff
At: ℓ_max ~ (Lex projection)
...
→ CKS coordinated allocation wrong
```

**Test 2: Spectral index not from ln(N)**
```
If n_s ≠ 1 - α/ln(N) at high precision
→ CKS discrete scaling incorrect
```
...
**In CMB:**
```
- First peak: ℓ₁ from W×a fundamental
- Peak spacing: Related to W harmonics
- Allocation batch: W × (coordination)
...
```
N = 9×10^60 (substrate size)
```

**In CMB:**
```
- Spectral index: n_s = 1 - α/ln(N)
- Total variance: σ² ~ 1/√N
- Correlation scales: K-space size ~ N^(1/3)
- Holographic projection: × N^(1/3) factor
...
- E-mode amplitude: ~ S factor of temperature
...
2. **Temperature anisotropies from registry allocation:**
   ...
   - Batch size B ~ 10^5 Lex
   - Holographic amplification
   
   Variance: σ/N ~ 1/√N_batch ~ 10^-27
   Projected: × N^(1/3) ~ 10^20 → 10^-5 ✓
...
3. **Acoustic peaks from hex-bus harmonics:**
   ...
   First peak: ℓ₁ ~ 220 from λ₀ = W×a = 42 mm
   Spacing: Δℓ ~ ℓ₁×(1 + 1/D) ~ 293 from D=3
...
6. **All C_ℓ spectrum from registry structure:**
   ```
   C_ℓ = Fourier[K-space correlation function]
   
   Components:
   - Low-ℓ: Integral constraint (1/ℓ²)
   - Peaks: Allocation harmonics (Σ sinc²(ℓ/ℓ_n))
   - Damping: Discrete resolution (exp(-ℓ²/ℓ_damp²))
...
8. **Redshift as holographic projection:**
   ...
   z IS:
   - Projection ratio (K-space → observation)
   - ω_K/ω_obs ~ 10^4
...
Spectral index: n_s = 1 - α/ln(N) (discrete)
...
```
Uniform temperature → Coordinated initialization (ω_s = constant)
Tiny fluctuations → Discrete allocation (Poisson ΔN/N ~ 10^-5)
...
Spectral tilt → Discrete scaling (n_s = 1 - α/ln N)
Low-ℓ anomalies → Initialization boundary (N=0 artifacts)
Redshift → Holographic projection (ω_K → ω_obs)
```
...
Map ≠ Territory
X-space (observation) ≠ K-space (reality)
CMB (rendering) ≠ Photons (don't exist)
```
...
- Push B-mode limits (confirm r ≈ 0)
...
```
ΔT/T ~ 10^-5 = Registry allocation noise
...
## Appendix: CMB Parameter Derivations

### Temperature Fluctuation Variance

```
Discrete allocation:
Batch size: B ~ W × (coordination) ~ 10^5 Lex

Number of batches:
N_batches = N_total/B = 10^60/10^5 = 10^55

Poisson variance:
ΔN/N ~ 1/√N_batches ~ 1/√(10^55) = 10^-27.5

Holographic amplification:
Factor ~ N^(1/3) ~ 10^20 (with projection factors)

Rendered variance:
(ΔT/T)_observed ~ 10^-27.5 × 10^22.5 = 10^-5 ✓

Measured: ΔT/T ~ 10^-5 (exact match!)
```
...
λ_K = W × a = 32 × 1.32 mm = 42.24 mm (K-space)

Holographic projection:
λ_obs ~ λ_K × (projection factors)
       ~ 42 mm × 10^25 (from N^(1/3) and cosmological)

Angular scale:
θ_1 ~ λ_obs/d_A ~ (resonance condition)
    ~ 0.8°

Multipole:
ℓ_1 = π/θ_1 ~ π/0.014 rad ~ 220 ✓

Measured: ℓ_1 ~ 220 (exact!)
```
