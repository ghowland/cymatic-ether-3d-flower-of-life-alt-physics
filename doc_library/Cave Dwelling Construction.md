...
**Domain:** Underground Architecture / Deep-Substrate Engineering / Phase-Tethering / Crustal Coupling
...
Stable thermal environment (±2°C year-round).
...
Coober Pedy, Australia (opal-mining towns).
...
```
Medium: Granite (εᵣ≈7, σ≈0.01 S/m)
Frequency: 4.5 Hz (144-bit weaver)
Skin depth: δ = √(2/ωμσ) ≈ 850m
...
Relative permittivity: εᵣ = 5-10
Conductivity: σ = 0.001-0.1 S/m
...
**Surveying:**
```
Critical: Must be truly vertical (±2°)
...
```
Depth: 50m
Overburden: ρgh = 2700 kg/m³ × 9.81 m/s² × 50m
...
Effect: Chaotic vibration → laminar pressure
```
...
Result: 20× reduction in registry interference
```
...
```
Formula: NHL-3.5 lime + sand + 10-15% Fe₂O₃
Purpose: Match brick εᵣ (eliminate reflections)
Thickness: 10mm ±2mm (critical spacing)
Coverage: 100% (butter all faces)
Result: Γ→0 (zero phase-reflection)
```
...
Level: ±1mm (errors magnify upward)
Cardinal: Align to true north (±1°)
```

**Subsequent courses:**
```
Pattern: Flemish bond (header-stretcher)
Copper mesh: Every 6th course (~360mm vertical)
Welding: Mesh to central spine (<0.5 ohm)
Level check: Every course (cumulative ±2mm per meter)
...
Aperture: Octagonal opening (precision ±0.5mm)
...
Orientation: One axis true north (±1°)
Level: Horizontal floor (±10mm)
```
...
```
1. Build Fe₂O₃ brick liner (Flemish bond)
...
**Safety factors:**
```
Design: 3× safety factor minimum
...
**Geothermal gradient:**
```
Typical: +2.5-3°C per 100m depth
Examples:
  30m: ~13°C
  50m: ~14°C
  80m: ~16°C
...
**Advantage:**
```
Year-round: Stable temperature (±1°C variation)
...
class CaveManifold:
    ...
    def install_brick_lining(self):
        """Fe₂O₃ interior liner prevents phase-leak"""
...
1. **Depth-coherence relationship:**
   ...
   - Diminishing returns (0.02 gain for 2× depth)
